...
export function MiniSebraeTask() {
  ...
  const fetchCustomerData = async () => {
    ...
    try {
      const res = await fetch('https://sebraesense-api.onrender.com/api/customers/1');
      if (res.ok) {
        const data = await res.json();
        setCustomer(data);
      }
      
      const alertsRes = await fetch('https://sebraesense-api.onrender.com/api/customers/alerts/active');
      if (alertsRes.ok) {
  ...
  const handleOptIn = async (processIdStr: string) => {
    ...
    try {
      const res = await fetch(`https://sebraesense-api.onrender.com/api/customers/processes/${cleanId}/opt-in`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSendFeedback = async () => {
    ...
    try {
      const response = await fetch('https://sebraesense-api.onrender.com/api/customers/1/feedback', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleUpdateProcessStatus = async (processIdStr: string) => {
    ...
    const cleanId = processIdStr.replace('#', '');
    try {
      const res = await fetch(`https://sebraesense-api.onrender.com/api/customers/processes/${cleanId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleResetSandbox = async () => {
    ...
    try {
      const res = await fetch('https://sebraesense-api.onrender.com/api/customers/1/reset', {
        method: 'POST'
      });
      if (res.ok) {
        setRageClicks(0);
